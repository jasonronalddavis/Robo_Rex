...
uint8_t ServoBus::_channelToGpioPin(uint8_t channel) const {
  ...
  if (channel >= PCA9685_FIRST_CHANNEL) return 0;
...
uint8_t ServoBus::_channelToPcaPort(uint8_t channel) const {
  if (channel < PCA9685_FIRST_CHANNEL || channel >= SERVO_COUNT) return 0;
  return channel - PCA9685_FIRST_CHANNEL;
}
...
bool ServoBus::begin(uint8_t i2c_addr, float freq_hz) {
  ...
  Serial.println(F("[ServoBus] GPIO: Allocated 2 LEDC timers"));
  ...
  if (i2c_error != 0) {
    ...
    Serial.print(F("[ServoBus] ERROR: I2C error code "));
    Serial.println(i2c_error);
    Serial.println(F("[ServoBus] Check: VCC, V+, wiring, address jumpers"));
    return false;
  }
  ...
  Serial.print(F("[ServoBus] PCA9685: Set to "));
  Serial.print(freq_hz);
  Serial.println(F(" Hz"));
  Serial.println(F("[ServoBus] ✓ Ready"));
  return true;
}
...
void ServoBus::attach(uint8_t channel, const ServoLimits& limits) {
  ...
  if (_isGpioChannel(channel)) {
    ...
    if (_gpioServos[channel].attach(pin, limits.minPulse, limits.maxPulse)) {
      ...
      Serial.print(F("[ServoBus] GPIO: ch"));
    ...
    _attached[channel] = true;
    Serial.print(F("[ServoBus] PCA9685: ch"));
    Serial.print(channel);
    Serial.print(F(" -> port "));
